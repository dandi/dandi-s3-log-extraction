...
def test_generate_summaries_with_data(tmp_path: pathlib.Path) -> None:
    ...
    # dataset1: asset1 has all 3 files; asset2 has only bytes_sent (no timestamps, no ips)
    ...
    # No ips.txt → exercises continue in _summarize_dataset_by_region
    ...
    # No ips.txt → summarized_activity_by_region stays empty → early return at line 286-287
...
def test_generate_all_dataset_totals_all_region_types(tmp_path: pathlib.Path) -> None:
    ...
    assert totals["dataset1"]["total_number_of_requests"] == "<50"
    assert totals["dataset1"]["total_number_of_downloads"] == "<50"
...
def test_generate_archive_totals_all_region_types(tmp_path: pathlib.Path) -> None:
    ...
    assert result["total_number_of_requests"] == "<50"
    assert result["total_number_of_downloads"] == "<50"
