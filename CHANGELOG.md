...
- Followed the upstream `s3-log-extraction` 1.11.2 release, which replaces the IPInfo and OpenCage APIs with a local GeoLite2-City database and ISO 3166 coordinate tables bundled with the package, writes region labels as the ISO 3166-1 alpha-3 country code and ISO 3166-2 subdivision code (`USA/CA` rather than `US/California`), and geolocates requesters while the summaries are generated instead of through an `ip_to_region.yaml` cache, so the `update ip regions` and `update ip refresh` steps are gone. `generate_dandiset_summaries` resolves each IP with the upstream `IpRegionResolver` (published cloud/VPN ranges first, then the local GeoLite2 database) and gains a `region_resolver` argument for supplying another resolver, such as the upstream `MappingRegionResolver`; the resolver is pickled into the worker processes, each of which opens its own database reader. The per-Dandiset and archive requester counts exclude cloud/VPN addresses by resolving them the same way. Running `dandis3logextraction update summaries` now needs network access for the service range listings and the `MAXMIND_ACCOUNT_ID` and `MAXMIND_LICENSE_KEY` credentials of a free MaxMind account on the first run and whenever the database is more than a week old, in place of `IPINFO_API_KEY` and `OPENCAGE_API_KEY`; `update ip coordinates` (from the upstream CLI) runs after the summaries and reads the published `by_region.tsv` files. The `geolocation` extra now installs `geoip2` and `requests` instead of `ipinfo` and `opencage`, and the upstream pin is raised to `>=1.11.2`. The tests that exercised upstream internals through the removed APIs are replaced by tests of the resolver behaviors and geolocation steps the DANDI pipeline relies on. ([#97](https://github.com/dandi/dandi-s3-log-extraction/pull/97))
