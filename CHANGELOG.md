...
- Followed the upstream `s3-log-extraction` 1.11.2 change that geolocates requesters while the summaries are generated and drops the `ip_to_region.yaml` cache with its `update ip regions` and `update ip refresh` steps. `generate_dandiset_summaries` resolves each IP with the upstream `IpRegionResolver` (published cloud/VPN ranges first, then the local GeoLite2 database) and gains a `region_resolver` argument for supplying another resolver, such as the upstream `MappingRegionResolver`; the resolver is pickled into the worker processes, each of which opens its own database reader. The per-Dandiset and archive requester counts exclude cloud/VPN addresses by resolving them the same way. Running `dandis3logextraction update summaries` now needs network access for the service range listings and the MaxMind credentials on the first run and whenever the database is more than a week old, while `update ip coordinates` (from the upstream CLI) runs after the summaries and reads the published `by_region.tsv` files. The upstream pin is raised to `>=1.11.2`. ([#97](https://github.com/dandi/dandi-s3-log-extraction/pull/97))
